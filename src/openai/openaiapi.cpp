...
std::string OpenaiApi::get(std::string route, std::map<std::string, std::string> params)
{
    ...
    std::string response;
    bool result = webclient.Get(url, headers, params, [&](const char *data, size_t len) {
...
void OpenaiApi::post_with_sse_response(std::string route, std::string body, std::deque<std::string> &event_queue)
{
    std::string url=base_url+route;
    bool result = webclient.Post(url, headers, body, [&](const char *data, size_t len) {
